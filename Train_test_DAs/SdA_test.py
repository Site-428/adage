...
def SdA_test(data_file, skip_col, network_file, net_structure):
    ...
    with open(network_file,'r') as network_fh:
        ...
        layer_para = []
        for each_layer in range(len(net_structure)):
            network_fh.readline() # skip the layer count line
            network_fh.readline() # skip 'weight matrix' line
            ...
            network_fh.readline() # skip 'hidden bias vector' line
            ...
            network_fh.readline() # skip 'visible bias vector' line
    ...
    with open(activity_file, 'w') as activity_fh:
        with open(raw_activity_file,'w') as raw_activity_fh:
            for each_layer in range(len(net_structure)):
                ...
                for each_sample in range(output.shape[0]):
